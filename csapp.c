...
void unix_error_nexit(char *msg) /* unix-style error without exiting */
{
    fprintf(stderr, "%s: %s\n", msg, strerror(errno));
    //exit(0);
}
...
ssize_t Rio_readn(int fd, void *ptr, size_t nbytes) 
{
    ...
    if ((n = rio_readn(fd, ptr, nbytes)) < 0)
	    unix_error_nexit("Rio_readn error");
    return n;
}
...
ssize_t Rio_writen(int fd, void *usrbuf, size_t n) 
{
    ...
    if ((rc = (rio_writen(fd, usrbuf, n))) != n)
        unix_error_nexit("Rio_writen error");
    return rc;
}
...
ssize_t Rio_readnb(rio_t *rp, void *usrbuf, size_t n) 
{
    ...
    if ((rc = rio_readnb(rp, usrbuf, n)) < 0)
        unix_error_nexit("Rio_readnb error");
    return rc;
}
...
ssize_t Rio_readlineb(rio_t *rp, void *usrbuf, size_t maxlen) 
{
    ...
    if ((rc = rio_readlineb(rp, usrbuf, maxlen)) < 0)
	    unix_error_nexit("Rio_readlineb error");
    return rc;
} 
...
int Open_clientfd_r(char *hostname, int port) 
{
    ...
    if ((rc = open_clientfd_r(hostname, port)) < 0) {
        unix_error_nexit("Open_clientfd_r error");
    }
    return rc;
}
